...
% Histogramme de la puissance de l'image en noir et blanc originale
figure, hist(LenaNB(:).^2, 50)
% Calcul de la somme des carrés des valeurs de l'image en noir et blanc
check1 = sum(LenaNB(:).^2);
% Affichage du titre avec la somme calculée
titre = sprintf('Check1  m0 = %5f', check1);
title(titre);

% Histogramme de la puissance de l'image reconstruite
figure, hist(abs(LenaFiltree(:).^2), 50)
% Calcul de la somme des carrés des valeurs de l'image reconstruite
check2 = sum(abs(LenaFiltree(:).^2));
% Affichage du titre avec la somme calculée
titre = sprintf('Check2  m0 = %5f', check2);
title(titre);
